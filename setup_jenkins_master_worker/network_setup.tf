...
#Initiate Peering connection request from ap-south-1
resource "aws_vpc_peering_connection" "apsouth1-uswest-2" {
  provider    = aws.region-master
  peer_vpc_id = aws_vpc.vpc_uswest.id
  vpc_id      = aws_vpc.vpc_apsouth.id
  #auto_accept = true
...
#Accept VPC peering request in us-west-2 from ap-south-1
resource "aws_vpc_peering_connection_accepter" "accept_peering" {
  provider                  = aws.region-worker
  vpc_peering_connection_id = aws_vpc_peering_connection.apsouth1-uswest-2.id
  auto_accept               = true
}

#Create route table in ap-south-1
...
#Create association between route table and subnet_1 in ap-south-1
...
#Create SG for allowing TCP/8080 from * and TCP/22 from your IP in ap-south-1
...
resource "aws_security_group" "jenkins-sg-oregon" {
  ...
  ingress {
    description = "Allow traffic from ap-south-1"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
